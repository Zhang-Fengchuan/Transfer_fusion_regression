classdef Initial_regression
    methods
        ...
        function [beta0, partitions, result, num_partitions_best,combine_principle_best,split_principle_best,random_number_best] = k_means_regression_initial(obj, num_partitions_list, eps_out,...
                iter_max_initial_out, combine_principle_list,...
                split_principle_list, random_number_list,...
                sample_size, p, x, y, disp_if, beta_real, c, high_if, CV_number, K_require, K_require_if, real_evaluate, t)
            %-----------------------------------------Functionality of the function-------------------------------------------%
            % The heterogeneous matrix regression model using the initial value search algorithm returns the number of clusters,
            ...
            result.random_number_best = [];
                [num_partitions_best,combine_principle_best,split_principle_best,random_number_best, cv_matrix] = obj.k_means_regression_cv(num_partitions_list,eps_out,...
                    iter_max_initial_out,combine_principle_list,split_principle_list,random_number_list,sample_size,p,x,y,disp_if,c,high_if,CV_number,K_require_if, real_evaluate, t);

                [beta0,coefficients,partitions,combine,residual_coefficients] = obj.k_means_regression(num_partitions_best,eps_out,...
                    iter_max_initial_out,combine_principle_best,split_principle_best,sample_size,p,x,y,random_number_best, high_if, CV_number,K_require_if, real_evaluate, t);
        ...
        function [beta_back,coefficients,...
                ...
                iter_max_initial_out,combine_principle,split_principle,sample_size,p,x,y,random_number, high_if, CV_number, K_require_if, real_evaluate, t)
            %------------------------------------------Functionality of the function---------------------------------------------%
            ...
            rng(random_number);
            if real_evaluate > 0
                index = randperm(sample_size + 1);
                partition_lengths = [floor((sample_size+1) / num_partitions) * ones(1, num_partitions - 1), (sample_size+1) - floor((sample_size+1) / num_partitions) * (num_partitions - 1)];
            ...
            if real_evaluate > 0
                for i = 1:num_partitions
                    % 在当前单元格中找到元素 t 的索引
                    idx = find(partitions{i} == t);
                    % 如果找到了元素 t，就将它删除
                    if ~isempty(idx)
                        partitions{i}(idx) = [];
                    end
                    % 将大于 t 的元素都减一
                    partitions{i}(partitions{i} > t) = partitions{i}(partitions{i} > t) - 1;
                end
            end
        ...
        function [beta_back,coefficients,...
                ...
                iter_max_initial_out,combine_principle,p,x,y,random_number,beta0,partitions_old,sample_size_sum,high_if,CV_number,K_require_if,real_evaluate,t)
            %------------------------------------------Functionality of the function---------------------------------------------%
            ...
            % if real_evaluate > 0
            %     %index = randperm(sample_size + 1);
            %     %partition_lengths = [floor(sample_size / num_partitions) * ones(1, num_partitions - 1), sample_size + 1 - floor(sample_size / num_partitions) * (num_partitions - 1)];
            %     partitions = t;
            % else
                index = randperm(sample_size);
                partition_lengths = [floor(sample_size / num_partitions) * ones(1, num_partitions - 1), sample_size - floor(sample_size / num_partitions) * (num_partitions - 1)];
